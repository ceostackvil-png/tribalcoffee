...
export default function App() {
  ...
  useEffect(() => {
    if (TRIBAL_PRODUCTS.length > 0 && cartItems.length === 0) {
      const initialItems: CartItem[] = [];
      if (TRIBAL_PRODUCTS[0]) {
        initialItems.push({ product: TRIBAL_PRODUCTS[0], quantity: 1 });
      }
      if (TRIBAL_PRODUCTS[3]) {
        initialItems.push({ product: TRIBAL_PRODUCTS[3], quantity: 2 });
      }
  ...
  return (
    <div className="relative min-h-screen w-full bg-espresso text-cream-latte grain-overlay">
      ...
      <CustomCursor isAdminActive={isAdminOpen} />
